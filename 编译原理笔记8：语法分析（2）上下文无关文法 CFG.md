...
前几篇博客中说到的词法分析，做的是从 【x+y → id+id】 的工作，也就是要将源代码变成一个记号流。语法分析，则要检查上面来的这个 id+id 记号流的语法结构是否正确，检查的方式就是“推导”（推导的概念会在本篇提到）。

若能够进行这样的推导： E=>E+E=>id+E=>id+id，即由 E 推出了 id+id，与一开始得到的 id+id 相同，则说明语法结构正确。
...
在推导的过程中，若每次直接推导均替换句型中最左边的非终结符，则成为最左推导。由最左推导产生的句型被称为左句型。
